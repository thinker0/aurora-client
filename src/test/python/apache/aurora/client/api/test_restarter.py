...
class TestRestarter(unittest.TestCase):
  ...
  def setUp(self):
    self.mock_instance_watcher = mock.Mock(spec=InstanceWatcher)
    self.mock_scheduler_proxy = mock.Mock(spec=SchedulerProxyApiSpec)
    self._expected_restart_calls = []
    self._expected_watch_calls = []
  ...
  def mock_restart_instances(self, instances):
    self.mock_scheduler_proxy.restartShards.return_value = make_response()
    self.mock_instance_watcher.watch.return_value = []
    self._expected_restart_calls.append(
      mock.call(JOB.to_thrift(), instances, retry=True))
    self._expected_watch_calls.append(mock.call(instances))
  ...
  def test_restart_one_iteration(self):
    ...
    self.mock_scheduler_proxy.restartShards.assert_has_calls(self._expected_restart_calls)
    self.mock_instance_watcher.watch.assert_has_calls(self._expected_watch_calls)
  ...
  def mock_status_active_tasks(self, instance_ids):
    ...
    self.mock_scheduler_proxy.getTasksWithoutConfigs.return_value = response
  ...
  def test_restart_no_instance_active(self):
    ...
    self.restarter.restart(None)
    assert self.mock_scheduler_proxy.restartShards.call_count == 0

  def mock_restart_fails(self):
    response = make_response(code=ResponseCode.ERROR, message='test error')
    self.mock_scheduler_proxy.restartShards.return_value = response
  ...
  def mock_restart_watch_fails(self, instances):
    self.mock_scheduler_proxy.restartShards.return_value = make_response()
    self.mock_instance_watcher.watch.return_value = instances
    self._expected_restart_calls.append(
      mock.call(JOB.to_thrift(), instances, retry=True))
    self._expected_watch_calls.append(mock.call(instances))
